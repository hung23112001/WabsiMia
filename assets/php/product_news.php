...
<body>
    ...
    <div id="container" style='padding-bottom:0'>
        ...
        <aside>
            <form method='post' action='./product_news.php?search' class="formSearch_product col-10">
            ...
            <?php
                echo "<div class='all_product col-12'>";
                    if(isset($_GET['numberPage'])){
                        ...
                        $all_product = "SELECT * FROM sanpham  ORDER BY maSP DESC LIMIT 6 OFFSET $product_start";
                    }
                    else{
                        if(isset($_GET['typeProduct'])){
                            if($_GET['typeProduct'] == "destiny"){
                                $all_product = "SELECT * FROM sanpham WHERE soLuong > '0' and loaiSP = 'Phong thủy' ORDER BY maSP DESC LIMIT 24 ";
                            }
                            else{
                                $all_product = "SELECT * FROM sanpham WHERE soLuong > '0' and loaiSP = 'Thời trang' ORDER BY maSP DESC LIMIT 24 ";
                            }
                        }
                        else if(isset($_GET['changePrice'])){
                            $getChangePrice = $_GET['changePrice'];
                            $all_product = "SELECT * FROM sanpham WHERE soLuong > '0' and  gia <= '{$getChangePrice}'  ORDER BY maSP DESC";
                        }
                            ...
                            if($select_typeSearch != "gia"){
                                $all_product = "SELECT * FROM sanpham WHERE soLuong > '0' and $select_typeSearch LIKE '%{$input_searchProduct}%' ORDER BY maSP DESC";
                            }
                            else{
                                $all_product = "SELECT * FROM sanpham  WHERE soLuong > '0' and $select_typeSearch <= '{$input_searchProduct}' ORDER BY maSP DESC";
                            }
                        }
                        else{
                            $all_product = "SELECT * FROM sanpham WHERE soLuong > '0' ORDER BY maSP DESC LIMIT 6 OFFSET 0";
                        }
                    }
                ...
                if(!isset($_GET['changePrice']) && !isset($_GET['search'])){
                    ...
                    echo "<div class='numberPage'>";
                        for($i = 0; $i <4; $i++){
                            $number = $i +1;
                            if($i == $page_current){
                                echo "<a href='./product_news.php?numberPage=$i'><p class='item_numberPage active_numberPage'>{$number}</p></a>";                                    
                            }
                            else{
                                echo "<a href='./product_news.php?numberPage=$i'><p class='item_numberPage'>{$number}</p></a>";
                            }
                        }
                    echo "</div>";
                }
